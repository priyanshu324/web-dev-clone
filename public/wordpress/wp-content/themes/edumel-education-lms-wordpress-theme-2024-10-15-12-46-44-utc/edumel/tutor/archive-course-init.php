...
if ( isset( $the_query ) ) {
	...
	if ( isset( $loop_content_only ) && true == $loop_content_only ) {
		echo edumel_wp_kses($course_loop); //phpcs:ignore --$course_loop contain sanitized data
		return;
	}
	...
	edumel_course_banner();

	?>



<div class="section-padding tutor-wrap tutor-wrap-parent tutor-courses-wrap tutor-container course-archive-page" data-tutor_courses_meta="<?php echo esc_attr( json_encode( $GLOBALS['tutor_course_archive_arg'] ) ); ?>">
	<?php if ( $has_course_filters && in_array( 'search', $supported_filters_keys ) ) : ?>
		<div class="tutor-d-block tutor-d-lg-none tutor-mb-32">
			<div class="tutor-d-flex tutor-align-center tutor-justify-between">
				<span class="tutor-fs-3 tutor-fw-medium tutor-color-black"><?php esc_html_e( 'Courses', 'edumel' ); ?></span>
	...
	<div class="tutor-row tutor-gx-xl-5">
		...
		<?php if ( $has_course_filters ) : ?>
			...
			<div class="tutor-col-xl-<?php echo edumel_wp_kses($columns) < 3 ? 8 : 9; ?> ">
				<div>
					<?php tutor_load_template( 'course-filter.course-archive-filter-bar' ); ?>
				</div>
				<div class="tutor-pagination-wrapper-replaceable" tutor-course-list-container>
					<?php echo edumel_wp_kses($course_loop); //phpcs:ignore --$course_loop contain sanitized data ?> 
				</div>
			</div>
		<?php else : ?>
