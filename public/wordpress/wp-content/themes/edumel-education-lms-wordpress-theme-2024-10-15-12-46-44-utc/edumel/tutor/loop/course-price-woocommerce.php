...
if ( ! $is_logged_in && ! $enable_guest_course_cart ) {
	...
	$enroll_btn = '<div class="tutor-course-list-btn">' . apply_filters( 'tutor_course_restrict_new_entry', '<a href="' . get_the_permalink() . '" class="tutor-btn tutor-btn-outline-primary tutor-btn-md tutor-btn-block ' . $required_loggedin_class . '">' . __( 'Enroll Course', 'edumel' ) . '</a>' ) . '</div>';
...
if ( tutor_utils()->is_course_purchasable() ) {
	...
	if ( false === $product ) {
		echo edumel_wp_kses($free_html); //phpcs:ignore --contain safe data
		...
		echo '<div class=" '. $utility_classes .' ">
                    ...
                    <div class="tutor-course-booking-progress tutor-d-flex tutor-align-center">
						...
						esc_html( $b_total ) . __( '% Booked', 'edumel' ) . '
                        </div>
                    </div>
                </div>
	...
	if ( $product && $maximum_students === $total_enrolled && 0 !== $maximum_students ) {
		...
		$restrict   = '<div class="list-item-booking booking-full tutor-d-flex tutor-align-center"><div class="booking-progress tutor-d-flex"><span class="tutor-mr-8 tutor-color-warning tutor-icon-circle-info"></span></div><div class="tutor-fs-7 tutor-fw-medium tutor-color-black">' . __( 'Fully Booked', 'edumel' ) . '</div></div></div>';
		echo edumel_wp_kses($price_html); //phpcs:ignore --contain safe data
		echo edumel_wp_kses($restrict); //phpcs:ignore --contain safe data
	}
	...
	if ( $product && 0 === $maximum_students ) {
		...
		echo edumel_wp_kses($price_html); //phpcs:ignore --contain safe data
		echo edumel_wp_kses($cart_html); //phpcs:ignore
	}
} else {
	echo edumel_wp_kses($free_html); //phpcs:ignore --contain safe data
}
